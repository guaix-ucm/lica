...
class Rect:
	...
	@classmethod
	def from_normalized(cls, width, height, n_roi, already_debayered=True):
		if n_roi.x0 is not None and n_roi.x0 + n_roi.w > 1.0:
			raise ValueError(f"normalized x0(={n_roi.x0}) + width(={n_roi.w}) = {n_roi.x0 + n_roi.w} exceeds 1.0")
		if n_roi.y0 is not None and n_roi.y0 + n_roi.h > 1.0:
			raise ValueError(f"normalized x0(={n_roi.y0}) + width(={n_roi.h}) = {n_roi.y0 + n_roi.h} exceeds 1.0")
